...
func testUserToManySnippets(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = randomize.Struct(seed, &b, snippetDBTypes, false, snippetColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, snippetDBTypes, false, snippetColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	...
	check, err := a.Snippets().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := UserSlice{&a}
	if err = a.L.LoadSnippets(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.Snippets); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.Snippets = nil
	if err = a.L.LoadSnippets(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.Snippets); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManyAddOpSnippets(t *testing.T) {
	var err error
	...
	foreigners := []*Snippet{&b, &c, &d, &e}
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, snippetDBTypes, false, strmangle.SetComplement(snippetPrimaryKeyColumns, snippetColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	foreignersSplitByInsertion := [][]*Snippet{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddSnippets(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.Snippets[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.Snippets[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.Snippets().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
...
func testUserToManySetOpSnippets(t *testing.T) {
	var err error
	...
	err = a.SetSnippets(ctx, tx, false, &b, &c)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.SetSnippets(ctx, tx, true, &d, &e)
	if err != nil {
		t.Fatal(err)
	}
...
func testUserToManyRemoveOpSnippets(t *testing.T) {
	var err error
	...
	err = a.AddSnippets(ctx, tx, true, foreigners...)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.RemoveSnippets(ctx, tx, foreigners[:2]...)
	if err != nil {
		t.Fatal(err)
	}
...
var (
	userDBTypes = map[string]string{`ID`: `bigint`, `Name`: `text`, `Email`: `text`, `Password`: `text`, `Created`: `date`}
	_           = bytes.MinRead
)
