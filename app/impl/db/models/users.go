...
type userR struct {
	Snippets SnippetSlice `boil:"Snippets" json:"Snippets" toml:"Snippets" yaml:"Snippets"`
}
...
var (
	userAllColumns            = []string{"id", "name", "email", "password", "created"}
	userColumnsWithoutDefault = []string{"name", "email", "password", "created"}
...
// Snippets retrieves all the snippet's Snippets with an executor.
func (o *User) Snippets(mods ...qm.QueryMod) snippetQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"snippets\".\"user_id\"=?", o.ID),
	)

	query := Snippets(queryMods...)
	queries.SetFrom(query.Query, "\"snippets\"")

	if len(queries.GetSelect(query.Query)) == 0 {
		queries.SetSelect(query.Query, []string{"\"snippets\".*"})
	}

	return query
}

// LoadSnippets allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadSnippets(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		qm.From(`snippets`),
		qm.WhereIn(`snippets.user_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if err != nil {
		return errors.Wrap(err, "failed to eager load snippets")
	}

	var resultSlice []*Snippet
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice snippets")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on snippets")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for snippets")
	}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if queries.Equal(local.ID, foreign.UserID) {
				local.R.Snippets = append(local.R.Snippets, foreign)
				if foreign.R == nil {
					foreign.R = &snippetR{}
				}
...
// AddSnippets adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.Snippets.
// Sets related.R.User appropriately.
func (o *User) AddSnippets(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Snippet) error {
	var err error
	...
	if o.R == nil {
		...
		o.R.Snippets = append(o.R.Snippets, related...)
	}
...
// SetSnippets removes all previously related items of the
// user replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.User's Snippets accordingly.
// Replaces o.R.Snippets with related.
// Sets related.R.User's Snippets accordingly.
func (o *User) SetSnippets(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Snippet) error {
	query := "update \"snippets\" set \"user_id\" = null where \"user_id\" = $1"
	...
	return o.AddSnippets(ctx, exec, insert, related...)
}

// RemoveSnippets relationships from objects passed in.
// Removes related items from R.Snippets (uses pointer comparison, removal does not keep order)
// Sets related.R.User.
func (o *User) RemoveSnippets(ctx context.Context, exec boil.ContextExecutor, related ...*Snippet) error {
	var err error
	for _, rel := range related {
		queries.SetScanner(&rel.UserID, nil)
		if rel.R != nil {
	...
	for _, rel := range related {
		for i, ri := range o.R.Snippets {
			...
			ln := len(o.R.Snippets)
			if ln > 1 && i < ln-1 {
				o.R.Snippets[i] = o.R.Snippets[ln-1]
			}
			o.R.Snippets = o.R.Snippets[:ln-1]
			break
		}
	}
